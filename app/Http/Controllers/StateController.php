...
class StateController extends Controller
{
    ...
    public function store(Request $request, CreateStateForm $form)
    {
        ...
        State::create($data);
        Splade::toast('Estado creado correctamente')->autoDismiss(3);
    ...
    public function update(Request $request, State $state, UpdateStateForm $form)
    {
        $data = $form->validate($request);
        $state->update($data);
        Splade::toast('Estado actualizado correctamente')->autoDismiss(3);
        return redirect()->route('admin.states.index');
    }
    ...
    {
        $state->delete();
        Splade::toast('Estado eliminado correctamente')->autoDismiss(3);
        return redirect()->route('admin.states.index');
    }
}
